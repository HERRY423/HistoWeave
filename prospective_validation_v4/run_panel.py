"""Run the locked nine-method CRC panel without reading pathology truth."""
...
def run_r_method(
    ...
    script = Path(__file__).with_name(f"run_{method}_v4.R")
    proc = subprocess.run(
        [str(rscript), "--vanilla", str(script), str(input_path), str(output_path), str(seed), str(q), str(r_lib)],
        capture_output=True,
        text=True,
        encoding="utf-8",
        errors="replace",
...
def estimate_k(adata: ad.AnnData) -> dict[str, object]:
    ...
    # No label field exists in the label-free input; guard against future drift.
...
def run_cell(
    ...
    if status_path.is_file():
        ...
        if existing.get("input_sha256") != input_sha256:
            raise RuntimeError(f"{sample_id}/{method}/{seed}: stale input hash")
        return existing
    ...
    try:
        if method in {"bayesspace", "banksy"}:
            ...
            if len(labels) != adata.n_obs:
                raise RuntimeError(f"{method}: label count does not match input")
            pd.DataFrame(
                {"spot_id": adata.obs_names.astype(str), "label": labels.astype(str)}
            ).to_csv(output_path, index=False)
    except Exception as exc:  # retain every failure
        status = "failed"
...
def main() -> None:
    ...
    parser.add_argument("--r-lib", type=Path, required=True)
    parser.add_argument(
        "--rscript",
        type=Path,
        default=Path("C:/R1/R-4.5.3/bin/Rscript.exe"),
    )
    ...
    if not set(samples).issubset(all_samples):
        raise ValueError("unknown sample requested")
    ...
    manifest = {
        "schema_version": "histoweave.crc.prediction_manifest.v4",
        "protocol_id": "histoweave-crc-patient-nonoracle-2026-07",
        "truth_accessed_by_runner": False,
        "methods_requested": methods,
        "samples_requested": samples,
        "seeds_requested": seeds,
        "cells": cells,
        "n_success": sum(row["status"] == "success" for row in cells),
        "n_failed": sum(row["status"] != "success" for row in cells),
        "finished_utc": datetime.now(UTC).isoformat(),
    }
