...
Across 20 grouped queries, always-personalising raised regret
...
A second outcome-sealed CRC study completed a same-mask nine-method panel in
seven patients, but its frozen actions remained evidence requests.
...
Across untouched studies, does admissible evidence justify personalisation? \\
...
closed without it. T5 remains construct validity only and does not substitute
for biological personalisation on an external study.
...
First, the historical five-dataset oracle-\(K\) landscape is descriptive and
is excluded from model validation and efficacy claims. Second, HER2ST and CRC
provide two outcome-sealed studies and 13 strict common-mask units, improving
independence but remaining below the 15--20 units sought for a strong positive
personalisation claim; confidence intervals remain wide. Third, 15 CRC
deep-method attempts were user-authorized runtime skips, leaving A120838 deep
scores based on one section and one seed and exposing substantial computational
reproducibility risk. Fourth, the external oracle panel lacks SpaGCN, STAGATE,
GraphST, and BayesSpace; only BANKSY overlaps the DLPFC spatial-method set.
Fifth, zero external personalisation coverage means risk--coverage frontiers
cannot yet demonstrate safer personalised operating points. Sixth,
registration is repository-timestamped rather than independently DOI-stamped.
Seventh, the adversarial corpus enumerates known contract violations but cannot
measure all future failure modes. Eighth, ISUS remains descriptive: the current
DLPFC calibration shows no reliable positive association with spatial ARI gain.
